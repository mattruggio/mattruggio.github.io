...
Direct3D 9
: Microsoft's 2002 graphics API, which most Windows games of its generation were built against, still supported on modern Windows but no longer a first-class path.
...
XInput
: The controller API introduced alongside the Xbox 360 in 2005, superseding DirectInput for gamepad input on Windows and exposing a fixed button and axis layout rather than a device-described one.
